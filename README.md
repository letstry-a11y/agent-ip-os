...
M0-00 through M0-05, M1-01 through M1-07, M2-00, M2-01, M2B-01, and M2-03 are complete; M2-04 is next. The repository has a pinned Python/Node workspace, locked quality gates, a verified six-service Docker Compose stack, shared versioned domain/Provider/Agent contracts, forward-only PostgreSQL migrations, cross-runtime canonical hashes, durable Temporal workflows, guarded Mock publish dispatch, two behaviorally distinct Provider Mock families, an accepted responsive human approval desk, and a versioned Xiaohongshu qualification record. The public [`letstry-a11y/agent-ip-os`](https://github.com/letstry-a11y/agent-ip-os) repository protects `main` with pull requests, an up-to-date passing `quality` check, resolved conversations, and linear history. G1 explicitly selects M2B while deployment region, legally usable Providers, and budget remain unapproved; G2 selects the M5B publish-package/manual-reconciliation fallback because no project test account, app, callback domain, exact generic note-publishing Scope, or API approval is evidenced.
...
- [M2-01 Provider-contract evidence](docs/acceptance/m2-01-provider-contracts.md): text/image/video/audio interfaces, deterministic primary Mocks, idempotent async lifecycle, cost/provenance/error/rate-limit evidence, and zero-network tests.
- [M2B-01 / M2-03 evidence](docs/acceptance/m2b-01-agent-contracts.md): alternate Provider behavior and routing plus exactly six fail-closed, zero-cost Agent unit contracts.
