...
| M0-03 | MVP PRD, system context, lifecycle, data model, and first ADRs | M0-02 | READY_FOR_REVIEW | [PRD](docs/product/prd-mvp.md), [context](docs/architecture/system-context.md), [data model](docs/architecture/data-model.md), [lifecycle](docs/specs/content-lifecycle.md), and accepted-baseline ADRs are internally consistent; founder records M0 review in [acceptance](docs/acceptance/m0.md). |
| M0-04 | Python/Node workspace and CI gates | M0-01, M0-03 accepted | READY | Pin supported runtime/dependency versions; establish format, lint, types, unit tests, secret scan, and migration check; all gates run locally and in CI from a clean checkout. |
| M0-05 | Docker Compose local environment | M0-04, virtualization/Docker prerequisites | BLOCKED | One documented command starts API, PostgreSQL, object storage, Temporal, and required UI/health checks; one documented command verifies health; restart and clean-checkout reproduction recorded. Blocked by missing Docker/WSL2 and unconfirmed virtualization. |
...
| M1-05 | Mock Agent, media, and platform adapters | M1-02 | NOT_STARTED | Deterministic fixtures cover success, invalid schema, timeout, transient/permanent failure, cancellation, lost response, and cost reporting with no network. |
| M1-06 | Minimal approval API/page | M1-03, M1-04 | NOT_STARTED | Human can inspect bound snapshot, approve/reject/request revision, and observe invalidation; E2E proves actor identity and no self-bypass. |
| M1-07 | Read-only platform qualification probe | D-001, D-007 input | NOT_STARTED | Official documentation, app/test-account status, callback domain, exact Scope, and minimal authorized probe result are recorded; absent Scope selects the package fallback without browser/Cookie automation. |
