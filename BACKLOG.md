...
| M0-04 | Python/Node workspace and CI gates | M0-01, M0-03 accepted | READY_FOR_REVIEW | uv/Python and Node/npm are pinned; lockfiles, API/web shells, repository/Python/web gates, and CI workflow exist; locked restore, `npm run check`, and `npm run build` pass in a fresh local clone, and the API smoke test passes. Hosted CI evidence remains pending D-009, so this task is not marked `DONE`. |
