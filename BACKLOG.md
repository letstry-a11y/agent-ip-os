...
| M0-05 | Docker Compose local environment | M0-04, Docker/WSL2 prerequisites | BLOCKED | Compose, safe local credentials, health checks, and one-command start/verify/restart operations are implemented. Completion still requires Docker/WSL2 installation plus recorded start, restart, and clean-checkout reproduction evidence on the target host. |
