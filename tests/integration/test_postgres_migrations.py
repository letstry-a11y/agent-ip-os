...
from psycopg.errors import ForeignKeyViolation, ObjectNotInPrerequisiteState, UniqueViolation
...
def _seed_approved_intent(database_url: str) -> dict[str, UUID]:
    ...
    candidate_hash = bytes.fromhex("41" * 32)
    ...
    with psycopg.connect(database_url, autocommit=True) as connection:
        ...
        connection.execute(
            ...
            INSERT INTO approval_snapshots (
                ...
                'PACKAGE_EXPORT', ARRAY[%s]::uuid[], CURRENT_TIMESTAMP + interval '1 day',
                CURRENT_TIMESTAMP, %s
            )
            """,
            (
